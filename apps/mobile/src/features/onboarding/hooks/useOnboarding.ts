...
import { getOnboardingByCooperativeId, getOnboardingSteps } from '../api/onboardingApi';
...
export function useOnboarding(cooperativeId: string) {
  ...
  const loadOnboarding = async () => {
    ...
    if (data) {
      ...
      if (stepsError) setError(stepsError.message);
      else if (stepsData) setSteps(stepsData);
    }
    setLoading(false);
  };

  return { onboarding, steps, loading, error, refetch: loadOnboarding };
}
